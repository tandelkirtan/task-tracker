    ...
    <div className="px-8 py-6 h-full flex flex-col overflow-hidden relative">
      ...
      {selectedDayTasks && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-bg-card rounded-[40px] w-full max-w-lg shadow-2xl overflow-hidden border border-border-main/50">
            <div className="p-8 border-b border-border-main/50 flex items-center justify-between bg-bg-input/30">
              <div>
                <h3 className="text-2xl font-bold text-text-primary">{format(selectedDayTasks.day, 'MMMM d, yyyy')}</h3>
                <p className="text-sm text-text-muted font-bold uppercase tracking-widest mt-1 opacity-70">Daily Schedule</p>
              </div>
              <button 
                onClick={() => setSelectedDayTasks(null)}
                className="p-3 bg-bg-card hover:bg-bg-input rounded-2xl transition-all shadow-sm border border-border-main/50 text-text-muted hover:text-text-primary"
              >
            ...
            <div className="p-8 max-h-[400px] overflow-y-auto custom-scrollbar space-y-4">
              {selectedDayTasks.tasks.length > 0 ? (
                selectedDayTasks.tasks.map(task => (
                  <div key={task.id} className="p-5 rounded-[24px] border border-border-main/50 bg-bg-input/30 flex items-center justify-between group hover:border-accent/30 hover:bg-bg-input transition-all">
                    <div className="flex items-center gap-4">
                      ...
                      <span className="font-bold text-text-primary group-hover:text-accent transition-colors">{task.title}</span>
                    </div>
                    <span className={cn(
                      "text-[10px] font-bold px-3 py-1.5 rounded-lg uppercase tracking-wider shadow-sm",
                ...
                <div className="py-16 text-center">
                  <div className="w-16 h-16 bg-bg-input rounded-full flex items-center justify-center mx-auto mb-4 border border-border-main/50 opacity-30">
                    <CalendarIcon className="w-8 h-8 text-text-muted" />
                  </div>
                  <p className="text-text-muted font-bold text-sm uppercase tracking-widest opacity-50">No tasks for today</p>
                </div>
              )}
            </div>

            <div className="p-8 bg-bg-input/30 border-t border-border-main/50">
              {!isPassed(selectedDayTasks.day) ? (
                <button 
                  ...
                  className="w-full py-4 bg-accent text-white font-bold rounded-[20px] shadow-xl shadow-accent/20 hover:brightness-110 transition-all flex items-center justify-center gap-2 active:scale-[0.98]"
                >
                  <Plus className="w-5 h-5" /> Add New Task
                </button>
              ) : (
                <div className="text-center text-text-muted text-[11px] font-bold uppercase tracking-widest opacity-50 flex items-center justify-center gap-2 bg-bg-input p-4 rounded-2xl border border-border-main/50">
                  <Info className="w-4 h-4" /> Past date - View only mode
                </div>
              )}
            </div>
          </div>
